 ...
 *
 * microSD card on the board's SDMMC peripheral (4-bit SDIO). Binds it to the
 * NuttX MMC/SD block driver -> /dev/mmcsd0, which stm32_bringup() then mounts
 * as FAT at /fs/microsd (logs, parameters, config).
 *
 * Neither supported board has a card-detect line, so we simply report
...
/* Only one SDMMC peripheral is enabled, so the driver numbers its slot 0. */
 ...
 *
 * Description:
 *   Initialize the selected SDMMC instance and bind it to the MMC/SD driver,
 *   /dev/mmcsd0.
 *
 ****************************************************************************/
...
int stm32_sdio_initialize(void)
{
  ...
  if (ret < 0)
    {
      mcerr("ERROR: Failed to bind SDMMC to MMC/SD driver: %d\n", ret);
      return ret;
    }
