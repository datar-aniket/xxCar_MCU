...
struct vst { uint64_t t,ts,ats,wts; float p[3],q[4],v[3],w[3],slip,a[3],torque,steer,speed; uint8_t status,rst,src,pad[5]; };
#pragma pack(pop)
int main(void){
  f=fopen("all.ulg","wb");
  ...
  const char*F[]={
    ...
    "vehicle_state_tx:uint64_t timestamp;uint64_t timestamp_sample;uint64_t accel_timestamp_sample;uint64_t wire_timestamp_us;float[3] position;float[4] quaternion;float[3] velocity;float[3] angular_velocity;float side_slip_rad;float[3] accel;float wheel_torque_nm;float steering_angle;float motor_speed_ms;uint8_t solution_status;uint8_t reset_counter;uint8_t source_valid;"};
  ...
  for(int k=0;k<4;k++){
    ...
    struct vst vs={.t=1010+k*500,.ts=900+k*500,.ats=990+k*500,.wts=1700000000000000ull+k*500,.p={1.0f,2.0f,3.0f},.q={1.0f},.a={0.03f,0.08f,-0.01f},.status=0x61,.src=0x0f}; dat(11,&vs,115);
  }
  fclose(f); return 0;
}
